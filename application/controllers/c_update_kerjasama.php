...
class C_update_kerjasama extends CI_Controller {
	...
	public function edit($id){
		$where = $id;
		$data['user'] = $this->db->query("SELECT a.ID_LEMBAGA, a.DESKRIPSI_KERJASAMA, a.TANGGAL_MULAI_KERJASAMA, a.TANGGAL_SELESAI_KERJASAMA, b.NAMA_LEMBAGA,
		c.NAMA_PERWAKILAN_LEMBAGA, a.STATUS_KERJASAMA FROM tb_mou_rf_kerjasama a join tb_mou_rf_lembaga b on a.ID_LEMBAGA = b.ID_LEMBAGA join tb_mou_rf_perwakilanlembaga c on b.ID_LEMBAGA = c.ID_LEMBAGA where a.ID_KERJASAMA = $where")->result_array();
		$data['id'] = $id;
		$this->load->template('view_update_kerjasama',$data);
	}
	...
	public function update($id){
		...
		$berkas_moa=$this->input->post('berkas_moa');
		$id_kodeberkasmoa=$this->input->post('id_kodeberkasmoa');
		$id_kerjasama=$this->input->post('id_kerjasama');
		$nama_berkas=$this->input->post('nama_berkas');
		$statuskerjasama=$this->input->post('statuskerjasama');
		...
		$this->m_input_kerjasama->update_data($where, $data_insert, 'tb_mou_rf_kerjasama');
		redirect('c_data_kerjasama');
